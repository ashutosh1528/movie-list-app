...
export const useQueryUtility = <TPayload = unknown, TResponse = unknown, TTransformResponse = TResponse>({
  url,
  payload,
  queryConfig,
  tranformFunction,
  ...
  url: string;
  payload: TPayload;
  queryConfig: QueryConfig<TTransformResponse>;
  tranformFunction?: (data: TResponse) => TTransformResponse;
}) => {
